...
{
    public partial class Manage_CommendFlag : System.Web.UI.Page
    {
	    ...
	    private void BindData()
	    {
            ...
            sql += " order by Commendflag desc,CommendSortID asc,id desc";

		    int total = 0;
		    int PageIndex = ConfigParam.Page;
		    int PageSize = grid.PageSize;
		    GK2010.BLL.Program bll = new GK2010.BLL.Program();
            List<GK2010.Model.Program> models = bll.GetList(PageSize, PageIndex, sql, "AdminParam", out total);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
	    {
		    for (int i = 0; i <= grid.Rows.Count - 1; i++)
		    {
                ...
                Model.Program model = bll.GetModel(ID);
                if (model != null)
                {
                    model.CommendSortID = DecimalHelper.Parse(txtSortID.Text, 0);
                    bll.Update(model);
                }
		    }
