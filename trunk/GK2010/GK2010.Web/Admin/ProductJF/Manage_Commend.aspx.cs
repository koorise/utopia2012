...
namespace GK2010.Web.Admin.ProductJF
{
    public partial class Manage_Commend : System.Web.UI.Page
    {
	    ...
	    private void BindData()
	    {
		    ...
		    List<GK2010.Model.ProductJF> models = bll.GetList(PageSize, PageIndex, ConfigParam.Keyword, "Commend", out total);
	    ...
	    {
            string UrlRedirect = "Manage_Commend.aspx?Keyword={0}";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            for (int i = 0; i <= grid.Rows.Count - 1; i++)
            {
                ...
                if (model != null)
                {
                    model.CommendSortID = DecimalHelper.Parse(txtSortID.Text, 2000);
                    bll.Update(model);
                }
            }
