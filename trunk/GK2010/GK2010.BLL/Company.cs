...
namespace GK2010.BLL
{
	...
	{
		#region  私有变量
		private readonly GK2010.DAL.Company dal=new GK2010.DAL.Company();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.Company model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.Company model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.Company GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.Company> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
        ...
        /// 得到默认对象实体
        /// </summary>
        public GK2010.Model.Company GetModelDefault()
        {
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.Company GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.Company model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.Company GetModelByCache(int ID)
		{
			...
			return (GK2010.Model.Company)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.Company> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.Company> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.Company> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.Company> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.Company> modelList = new List<GK2010.Model.Company>();
			GK2010.Model.Company model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.Company();
				if(dr["ID"].ToString()!="")
				{
