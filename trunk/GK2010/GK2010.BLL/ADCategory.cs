...
{
	/// <summary>
	/// 业务逻辑类ADCategory 的摘要说明。
	/// </summary>
	public class ADCategory
	{
		#region  私有变量
		private readonly GK2010.DAL.ADCategory dal=new GK2010.DAL.ADCategory();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.ADCategory model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.ADCategory model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.ADCategory GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.ADCategory> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.ADCategory GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.ADCategory model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.ADCategory GetModelByCache(int ID)
		{
			
			string CacheKey = "ADCategoryModel-" + ID;
			...
			return (GK2010.Model.ADCategory)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.ADCategory> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.ADCategory> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ADCategory> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ADCategory> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.ADCategory> modelList = new List<GK2010.Model.ADCategory>();
			GK2010.Model.ADCategory model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.ADCategory();
				if(dr["ID"].ToString()!="")
				{
        ...
        #region 重新组合
        public List<GK2010.Model.ADCategory> GetListNew()
        {
            List<GK2010.Model.ADCategory> models_new = new List<GK2010.Model.ADCategory>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ADCategory> models = DataTableToList(dt.Select("ParentID=0", "SortID ASC"));
            foreach (GK2010.Model.ADCategory model in models)
            {
        ...
        private void GetListNew(int ParentID, List<GK2010.Model.ADCategory> models_new, DataTable dt)
        {
            List<GK2010.Model.ADCategory> models = DataTableToList(dt.Select("ParentID=" + ParentID, "SortID ASC"));
            foreach (GK2010.Model.ADCategory model in models)
            {
                models_new.Add(model);
                GetListNew(model.ID, models_new, dt);
        ...
        public List<GK2010.Model.ADCategory> GetListDropDownList(int CurrentID)
        {
            List<GK2010.Model.ADCategory> models_new = new List<GK2010.Model.ADCategory>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ADCategory> models = DataTableToList(dt.Select("ParentID=0 and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
        ...
        private void GetListDropDownList(int CurrentID, int ParentID, List<GK2010.Model.ADCategory> models_new, DataTable dt)
        {
            //不包括：当前类别和以当前类别为父的类别
            List<GK2010.Model.ADCategory> models = DataTableToList(dt.Select("ParentID=" + ParentID + " and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
            foreach (GK2010.Model.ADCategory model in models)
            {
