...
namespace GK2010.BLL
{
   ...
   {
       #region 私有变量
       private readonly GK2010.DAL.MemberCart mc = new DAL.MemberCart();
       #endregion

       #region 增加一条数据
       public int Add(Model.MemberCart model) {
           return mc.Add(model);
       }
       #endregion

       #region 根据用户id返回数据,得到一个实体
       public Model.MemberCart GetModelByUserID(int userID)
       {
           List<Model.MemberCart> list = DataTableToList(mc.GetDataSetByUserID(userID).Tables[0]);
           if (list.Count > 0)
               return list[0];
           else
               return null;
       ...
       public Model.MemberCart GetModelByCartNum(string cartNum)
       {
           List<Model.MemberCart> list = DataTableToList(mc.GetDataSetByCartNum(cartNum).Tables[0]);
           if (list.Count > 0)
               return list[0];
           else
               return null;
       ...
       /// 得到一个对象实体
       /// </summary>
       public Model.MemberCart GetModelByCartNumAndUserID(string cartNum, int userID)
       {
           ...
           Model.MemberCart model = new Model.MemberCart();

           DataSet ds = DbHelperSQL.RunProcedure("UP_MemberCartConsignee_GetModelByCartNumAndUserID", parameters, "ds");
       ...
       public bool UpdateMemberCartByPart(Model.MemberCart mod_mc){
           return mc.UpdateMemberCartByPart(mod_mc);
       }
       #endregion
       ...
       /// 转换数据列表
       /// </summary>
       public List<GK2010.Model.MemberCart> DataTableToList(DataTable dt)
       {
       ...
       /// 转换数据列表
       /// </summary>
       public List<GK2010.Model.MemberCart> DataTableToList(DataRow[] drs)
       {
           List<GK2010.Model.MemberCart> modelList = new List<GK2010.Model.MemberCart>();
           GK2010.Model.MemberCart model;
           foreach (DataRow dr in drs)
           {
               model = new GK2010.Model.MemberCart();
               if (dr["Id"].ToString() != "")
               {
      ...
      #region 收件人收货信息
       ...
       public static string saveForm_Consignee()
       {
           ...
           DAL.MemberCart dal =  new DAL.MemberCart();
           Model.MemberCart model = bll.GetModelByCartNum(CookieHelper.GetCookies("cartNum"));
       ...
       public static string closeForm_Consignee()
       {
           ...
           BLL.MemberCart bll = new MemberCart();
           Model.MemberCart model = bll.GetModelByCartNum(Utility.CookieHelper.GetCookies("cartNum"));
