...
{
	/// <summary>
	/// 业务逻辑类HelpCategory 的摘要说明。
	/// </summary>
	public class HelpCategory
	{
		#region  私有变量
		private readonly GK2010.DAL.HelpCategory dal=new GK2010.DAL.HelpCategory();
		#endregion
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.HelpCategory GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.HelpCategory> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.HelpCategory GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.HelpCategory model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
        ...
        {
            return GetTitle(int.Parse(ID.ToString()));
        }
		#endregion
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.HelpCategory GetModelByCache(int ID)
		{
			
			string CacheKey = "HelpCategoryModel-" + ID;
			...
			return (GK2010.Model.HelpCategory)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.HelpCategory> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.HelpCategory> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.HelpCategory> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.HelpCategory> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.HelpCategory> modelList = new List<GK2010.Model.HelpCategory>();
			GK2010.Model.HelpCategory model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.HelpCategory();
				if(dr["ID"].ToString()!="")
				{
