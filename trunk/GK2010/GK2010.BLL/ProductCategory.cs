...
{
	/// <summary>
	/// 业务逻辑类ProductCategory 的摘要说明。
	/// </summary>
	public class ProductCategory
	{
		#region  私有变量
		private readonly GK2010.DAL.ProductCategory dal=new GK2010.DAL.ProductCategory();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.ProductCategory model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.ProductCategory model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.ProductCategory GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.ProductCategory> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.ProductCategory GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.ProductCategory model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.ProductCategory GetModelByCache(int ID)
		{
			
			string CacheKey = "ProductCategoryModel-" + ID;
			...
			return (GK2010.Model.ProductCategory)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.ProductCategory> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.ProductCategory> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ProductCategory> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ProductCategory> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.ProductCategory> modelList = new List<GK2010.Model.ProductCategory>();
			GK2010.Model.ProductCategory model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.ProductCategory();
				if(dr["ID"].ToString()!="")
				{
				...
				model.ParamUnit=dr["ParamUnit"].ToString();
				modelList.Add(model);
			}
			return modelList;
		}
        ...
        #region 重新组合
        public List<GK2010.Model.ProductCategory> GetListNew()
        {
            List<GK2010.Model.ProductCategory> models_new = new List<GK2010.Model.ProductCategory>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ProductCategory> models = DataTableToList(dt.Select("ParentID=0", "SortID ASC"));
            foreach (GK2010.Model.ProductCategory model in models)
            {
        ...
        private void GetListNew(int ParentID, List<GK2010.Model.ProductCategory> models_new, DataTable dt)
        {
            List<GK2010.Model.ProductCategory> models = DataTableToList(dt.Select("ParentID=" + ParentID, "SortID ASC"));
            foreach (GK2010.Model.ProductCategory model in models)
            {
                models_new.Add(model);
                GetListNew(model.ID, models_new, dt);
        ...
        public List<GK2010.Model.ProductCategory> GetListDropDownList(int CurrentID)
        {
            List<GK2010.Model.ProductCategory> models_new = new List<GK2010.Model.ProductCategory>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ProductCategory> models = DataTableToList(dt.Select("ParentID=0 and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
        ...
        private void GetListDropDownList(int CurrentID, int ParentID, List<GK2010.Model.ProductCategory> models_new, DataTable dt)
        {
            //不包括：当前类别和以当前类别为父的类别
            List<GK2010.Model.ProductCategory> models = DataTableToList(dt.Select("ParentID=" + ParentID + " and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
            foreach (GK2010.Model.ProductCategory model in models)
            {
