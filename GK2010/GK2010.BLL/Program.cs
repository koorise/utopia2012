...
namespace GK2010.BLL
{
	...
	{
		#region  私有变量
		private readonly GK2010.DAL.Program dal=new GK2010.DAL.Program();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.Program model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.Program model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.Program GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.Program> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.Program GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.Program model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.Program GetModelByCache(int ID)
		{
			
			string CacheKey = "ProgramModel-" + ID;
			...
			return (GK2010.Model.Program)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.Program> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.Program> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.Program> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.Program> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.Program> modelList = new List<GK2010.Model.Program>();
			GK2010.Model.Program model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.Program();
				if(dr["ID"].ToString()!="")
				{
		...
		/// 获得数据列表
		/// </summary>
        public List<GK2010.Model.Program> GetList(int PageSize, int PageIndex, int ApplicationFlag, int FirstCategoryID, int SecondCategoryID, int OrderType, out int totalRows)
		{
            DataSet ds = dal.GetList(PageSize, PageIndex, ApplicationFlag, FirstCategoryID,SecondCategoryID,OrderType,out totalRows);
			return DataTableToList(ds.Tables[0]);
		}
		#endregion
