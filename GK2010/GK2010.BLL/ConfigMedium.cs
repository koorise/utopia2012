...
{
	/// <summary>
	/// 业务逻辑类ConfigMedium 的摘要说明。
	/// </summary>
	public class ConfigMedium
	{
		#region  私有变量
		private readonly GK2010.DAL.ConfigMedium dal=new GK2010.DAL.ConfigMedium();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.ConfigMedium model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.ConfigMedium model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.ConfigMedium GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.ConfigMedium> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.ConfigMedium GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.ConfigMedium model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.ConfigMedium GetModelByCache(int ID)
		{
			
			string CacheKey = "ConfigMediumModel-" + ID;
			...
			return (GK2010.Model.ConfigMedium)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.ConfigMedium> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.ConfigMedium> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ConfigMedium> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ConfigMedium> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.ConfigMedium> modelList = new List<GK2010.Model.ConfigMedium>();
			GK2010.Model.ConfigMedium model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.ConfigMedium();
				if(dr["ID"].ToString()!="")
				{
        ...
        #region 重新组合
        public List<GK2010.Model.ConfigMedium> GetListNew()
        {
            List<GK2010.Model.ConfigMedium> models_new = new List<GK2010.Model.ConfigMedium>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ConfigMedium> models = DataTableToList(dt.Select("ParentID=0", "SortID ASC"));
            foreach (GK2010.Model.ConfigMedium model in models)
            {
        ...
        private void GetListNew(int ParentID, List<GK2010.Model.ConfigMedium> models_new, DataTable dt)
        {
            List<GK2010.Model.ConfigMedium> models = DataTableToList(dt.Select("ParentID=" + ParentID, "SortID ASC"));
            foreach (GK2010.Model.ConfigMedium model in models)
            {
                models_new.Add(model);
                GetListNew(model.ID, models_new, dt);
        ...
        public List<GK2010.Model.ConfigMedium> GetListDropDownList(int CurrentID)
        {
            List<GK2010.Model.ConfigMedium> models_new = new List<GK2010.Model.ConfigMedium>();
            DataTable dt = dal.GetList("", "").Tables[0];
            List<GK2010.Model.ConfigMedium> models = DataTableToList(dt.Select("ParentID=0 and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
        ...
        private void GetListDropDownList(int CurrentID, int ParentID, List<GK2010.Model.ConfigMedium> models_new, DataTable dt)
        {
            //不包括：当前类别和以当前类别为父的类别
            List<GK2010.Model.ConfigMedium> models = DataTableToList(dt.Select("ParentID=" + ParentID + " and ID<>" + CurrentID + " and ParentID <>" + CurrentID, "SortID ASC"));
            foreach (GK2010.Model.ConfigMedium model in models)
            {
