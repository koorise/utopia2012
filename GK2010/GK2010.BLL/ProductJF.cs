...
{
	/// <summary>
	/// 业务逻辑类ProductJF 的摘要说明。
	/// </summary>
	public class ProductJF
	{
		#region  私有变量
		private readonly GK2010.DAL.ProductJF dal=new GK2010.DAL.ProductJF();
		#endregion
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.ProductJF GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.ProductJF> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.ProductJF GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.ProductJF model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.ProductJF GetModelByCache(int ID)
		{
			
			string CacheKey = "ProductJFModel-" + ID;
			...
			return (GK2010.Model.ProductJF)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.ProductJF> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.ProductJF> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ProductJF> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ProductJF> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.ProductJF> modelList = new List<GK2010.Model.ProductJF>();
			GK2010.Model.ProductJF model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.ProductJF();
				if(dr["ID"].ToString()!="")
				{
