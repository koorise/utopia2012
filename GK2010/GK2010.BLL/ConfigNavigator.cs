...
{
	/// <summary>
	/// 业务逻辑类ConfigNavigator 的摘要说明。
	/// </summary>
	public class ConfigNavigator
	{
		#region  私有变量
		private readonly GK2010.DAL.ConfigNavigator dal=new GK2010.DAL.ConfigNavigator();
		#endregion
		...
		/// 增加一条数据
		/// </summary>
		public int  Add(GK2010.Model.ConfigNavigator model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(GK2010.Model.ConfigNavigator model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public GK2010.Model.ConfigNavigator GetModel(int ID)
		{
			
			DataSet ds = dal.GetModel(ID);
			List<GK2010.Model.ConfigNavigator> models = DataTableToList(ds.Tables[0]);
			if (models.Count > 0)
			{
				return models[0];
			}
        ...
        /// 得到默认对象实体
        /// </summary>
        public GK2010.Model.ConfigNavigator GetModelDefault()
        {

            DataSet ds = dal.GetModelDefault();
            List<GK2010.Model.ConfigNavigator> models = DataTableToList(ds.Tables[0]);
            if (models.Count > 0)
            {
                return models[0];
		...
		/// 得到一个对象实体(关键字)
		/// </summary>
		public GK2010.Model.ConfigNavigator GetModel(string Keywords)
		{
		...
		public static string GetTitle(int ID)
		{
			...
			GK2010.Model.ConfigNavigator model = bll.GetModel(ID);
			if (model != null)
			{
				return model.Title;
			}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public GK2010.Model.ConfigNavigator GetModelByCache(int ID)
		{
			
			string CacheKey = "ConfigNavigatorModel-" + ID;
			...
			return (GK2010.Model.ConfigNavigator)objModel;
		}
		#endregion
		...
		/// 列表(不分页)
		/// </summary>
		public List<GK2010.Model.ConfigNavigator> GetList(string Keywords, string Type)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GK2010.Model.ConfigNavigator> GetList(int PageSize, int PageIndex, string Keywords,string Type, out int totalRows)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ConfigNavigator> DataTableToList(DataTable dt)
		{
		...
		/// 转换数据列表
		/// </summary>
		public List<GK2010.Model.ConfigNavigator> DataTableToList(DataRow[] drs)
		{
			List<GK2010.Model.ConfigNavigator> modelList = new List<GK2010.Model.ConfigNavigator>();
			GK2010.Model.ConfigNavigator model;
			foreach (DataRow dr in drs)
			{
				model = new GK2010.Model.ConfigNavigator();
				if(dr["ID"].ToString()!="")
				{
