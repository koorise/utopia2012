<%@ Page Language="C#" MasterPageFile="~/ZSiteBaseAdmin.Master" AutoEventWireup="true" CodeBehind="Manage.aspx.cs" Inherits="GK2010.Web.Admin.Member.Manage" Title="" %>
...
<div id="divSearch" class="admin_table mb10">
	<asp:Panel ID="PanelSearch" runat="server" DefaultButton="btnSearch">
		用户名：<asp:TextBox ID="txtKey" runat="server" cssclass="input input_wa" ></asp:TextBox>
		<asp:Button ID="btnSearch" runat="server" cssclass="btn_gray_small" OnClick="btnSearch_Click" Text="搜索" />
	</asp:Panel>
</div>
<h2 class="h1"><span class="mr10 fl">注册会员管理列表</span></h2>
	...
	<asp:GridView ID="grid" runat="server" OnRowDataBound="grid_RowDataBound" Width="100%" BorderWidth="0" AutoGenerateColumns="false" GridLines="None" >
		<Columns>
			...
			<asp:TemplateField HeaderText="城市">
				<ItemTemplate><%# GK2010.BLL.ConfigCity.CityName( Eval("CityID").ToString()) %></ItemTemplate>
				<ItemStyle cssclass="td2" />
			</asp:TemplateField> 	
			...
			<asp:TemplateField HeaderText="订单数">
				<ItemTemplate><%# Eval("TotalOrder") %></ItemTemplate>
				<ItemStyle cssclass="td2" />
			</asp:TemplateField> 
			<asp:TemplateField HeaderText="积分">
				<ItemTemplate><%# Eval("TotalJF")%></ItemTemplate>
				<ItemStyle cssclass="td2" />
			</asp:TemplateField> 
			...
			<asp:TemplateField HeaderText="操作" >
				<ItemTemplate><asp:HyperLink ID="HyperLinkModify" runat="server">[查看]</asp:HyperLink><asp:HyperLink ID="HyperLinkDelete" runat="server">[删除]</asp:HyperLink></ItemTemplate>
				<ItemStyle cssclass="td2" />
			</asp:TemplateField>
		</Columns>
